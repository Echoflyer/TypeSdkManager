                    ...
                    <asp:ListView ID="ListViewIOS" runat="server" DataSourceID="SqlDataSourceIOS">
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td><%#Eval("PlatformName") %></td>
                                <td><%#Eval("Version") %><input type="hidden" value='<%#Eval("pvid") %>' name="hfpvid" /></td>
                                <td><a class="btn btn-primary btn-sm" href='GamePlatformConfig.aspx?gameid=<%#Eval("GameID") %>&platformid=<%#Eval("platformid") %>&SignatureKeyID=<%#Eval("SignatureKeyID") %>&systemid=2&gamedisplayname=<%#Eval("GameDisplayName") %>&gamename=<%#Eval("GameName") %>&platformname=<%#Eval("PlatformName") %>&androidversionid=<%=androidversionid %>&iosversionid=<%=iosversionid %>&pluginid=0'><i class='fa fa-edit'></i> 参数配置</a></td>
                                <td><a class="btn btn-primary btn-sm" href='GamePlatformIcon.aspx?gameid=<%#Eval("GameID") %>&platformid=<%#Eval("platformid") %>&SignatureKeyID=<%#Eval("SignatureKeyID") %>&systemid=2&gamedisplayname=<%#Eval("GameDisplayName") %>&gamename=<%#Eval("GameName") %>&platformname=<%#Eval("PlatformName") %>&androidversionid=<%=androidversionid %>&iosversionid=<%=iosversionid %>&pluginid=0'><i class='fa fa-edit'></i> 图标配置</a></td>
                            </tr>
                        </ItemTemplate>
                    </asp:ListView>
                </div>
